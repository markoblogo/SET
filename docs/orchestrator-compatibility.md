...
`context_package.capability_profile` records the selected profile and its exact exports. Profiles are registry-selected, planning-only, and do not install dependencies, grant tool authority, or enable a runtime. `baseline` exports context and loop review hints; `research` adds diversity and optional project-memory guidance; `governed-runner` adds optional memory and shadow-first governance contracts; `loop-readiness` adds a disabled L1/L2 report-first contract; `loop-hardening` adds disabled harness-stripping, runtime-path sprint, and broken-window revalidation rules; `bounded-orchestration` adds a disabled planner-review-executor handoff contract; `ship-router` adds a disabled delivery-lane routing contract for direct, review-first, bounded-loop, and human-gated work; `git-native-context` adds a disabled minimal typed-document lifecycle and relation contract; `bug-evidence` adds a disabled captured red-to-green evidence contract; `design-taste-review` adds disabled marketing/editorial routing, preservation-first redesign review, and browser evidence requirements; `agent-operations` adds disabled agent cards, async receipts, scoped memory, and evidence-backed provider/tool routing; `skill-quality-pipeline` adds a disabled staged-review contract for rollout evidence, bounded skill edits, held-out validation, rejected-edit memory, and `best_skill.md` export. See `docs/capability-profiles.md` and the linked contract documents.
